...
func simulate(ui BounceUI) {
	...
	go func() {
		for i := 0; i < 25; i++ {
			ui.ReceivedGroupMessage(Message{Destination: thread1.ID, Source: user1.ID, Text: "hello this is from user 1"})
			time.Sleep(1 * time.Second)
		}
	}()
	go func() {
		for i := 0; i < 10; i++ {
			ui.ReceivedGroupMessage(Message{Destination: thread2.ID, Source: user2.ID, Text: "hello this is from user 2"})
