			...
			if gs.isMember(b.currentUserID()) {
				if b.postNotification != nil {
					...
					if !deferToAnotherDevice && waitingForInitialSyncFrom == "" {
						b.postNotification(initialGroup.ID.String(), initialGroup.Name, "You have been added to a group")
					}
				}
			} else {
				if b.postNotification != nil {
					deferToAnotherDevice := !uiIsInForeground.Load() && anotherDeviceIsActive.Load()
					if !deferToAnotherDevice && waitingForInitialSyncFrom == "" {
						b.postNotification(initialGroup.ID.String(), initialGroup.Name, "You have been invited to a group")
					}
				}
			}
		} else {
