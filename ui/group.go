...
func (ui *ui) SetGroupState(bounceGroup chat.Group) {
	...
	fyne.DoAndWait(func() {
		...
		if wasAnInvite {
			ui.populateUpdateGroupHistory(g)
...
func (ui *ui) populateUpdateGroupHistory(g *group) {
	state := ui.bounce.GetUpdateGroupHistory(g.id)
